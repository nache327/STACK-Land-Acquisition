...
|---|---|---|---|---|
| 2 | B | Randolph / Norfolk MA | **Unresolvable assessor codes (rebind can't resolve — MAPC too coarse).** MAPC's Randolph layer consolidates to 6 districts vs the ordinance's 13, so no clean spatial rebind; verdicts were keyed to existing parcel codes and 3 remain undecodable against the §4.6 Table of Uses: **`GPOD`** (16 parcels — reads as the *Great Pond Commerce Center Overlay* §10.5 code, which obscures the underlying base district), **`HA`** (3), **`C`** (1). The 14 clear codes (RH/RM/A/A55/R/CSBD/NRBD/WCBD/B/BP/SFD/I/BHRD/GBHD) are grounded + applied. | Decode `GPOD`/`HA`/`C`: confirm each parcel's underlying base district, or point a `randolph.json` rebind at the town's ArcGIS zoning layer with the full 13-district vocab. 20 parcels held. Source: eCode360 RA1509 Ch. 200 §3.1/§4.6. |
