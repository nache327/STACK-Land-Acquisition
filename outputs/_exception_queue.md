...
|---|---|---|---|---|
| B1 | B | Millis, Norfolk MA | Ordinance is a scanned-image PDF (no text layer); no OCR/renderer available. Not on eCode360 (MI3159 = Millvale PA — catch #38 avoided) or Municode. | Paste the use table, or run OCR on the town PDF. |
| B2 | B | Plainville, Norfolk MA | Bylaw auto-fetchable (eCode Ch.500) BUT MAPC returns 0 (SRPEDD region, not MAPC) → no polygon source to fill its 91%-NULL parcels. | A town/SRPEDD zoning GIS layer (polygons) to rebind against; then ground the fetchable bylaw. |
