...
|---|---|---|---|---|
| B2 | B | Plainville / Norfolk MA | **No queryable GIS layer to rebind (SRPEDD region — MAPC returns 0 for Plainville).** Bylaw IS auto-fetchable (eCode360 Ch. 500, `§ 500-17` Use/Intensity Schedule = Attachment 1), but parcels.zoning_code is **91% NULL** (2,644/2,914) and MAPC (Metro-Boston atlas) doesn't cover Plainville, so there's no polygon source to spatially fill the nulls. The ~270 coded parcels use `CB/CA/IB/IA/IC/CI/CC/CD/RC/RA/RB`. | Provide a Plainville zoning **GIS layer** (town ArcGIS or SRPEDD regional) so the 91% NULL parcels can be spatially rebound; then ground against eCode360 Ch. 500 §500-17. (Alternatively OK a verdict-only pass on the ~270 coded parcels + accept the null tail.) Source: ecode360.com/11814987 (Ch. 500). |
| B1 | B | Millis / Norfolk MA | **Ordinance is a scanned-image PDF (no text layer) and no renderer/OCR available.** Only source is the town Zoning Bylaw PDF (millisma.gov new-CMS shortlink `aptg.co/9n7GvH`, 231pp) — every page is a single image, 0 extractable text; `pdftoppm`/`pytesseract`/`pdf2image` all absent, so neither the Read renderer nor OCR can read the use table. NOT on eCode360 (MI3159 = Millvale PA, catch #38) or Municode. MAPC district scheme = C-V/I-P/R-S/R-T/R-V (I-P = Industrial Park needle). | Paste (or OCR) the Millis Table of Use Regulations + district-establishment section for districts C-V/I-P/R-S/R-T/R-V (esp. the I-P Industrial Park row's warehouse/self-storage/manufacturing status). Then rebind (MAPC layer 2, muni='Millis') + ground. Source: aptg.co/9n7GvH. |
