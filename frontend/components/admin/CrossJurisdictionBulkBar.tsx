...
export function CrossJurisdictionBulkBar({
  selectedCount,
    ...
    >
      <span className="text-sm font-medium text-slate-700">
        ...
        <span className="ml-1 text-xs font-normal text-slate-500">
          across {jurisdictionsTouched} jurisdiction
          {jurisdictionsTouched === 1 ? "" : "s"}
        </span>
      </span>
      ...
      <div className="ml-auto flex items-center gap-2 text-xs text-slate-500">
        <span title="The backend caps each /_bulk-review call at 50 source_ids; the client groups by jurisdiction then 50-row-chunks each group.">
          fan-out per juris × 50
        </span>
        <button
          type="button"
          onClick={onClear}
