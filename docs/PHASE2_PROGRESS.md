...
**Last 14d (sources: `backend/tmp/jobs_all_latest.json`, `backend/tmp/job_monomouth_08b0f866.json`, `backend/tmp/job_westchester_886141e2.json`, live admin jobs `d98324cc-6c78-4e2f-a6ab-d6fb15f92835`, `3b4582c5-e47e-4bc7-8d38-7853e0173a89`, `a9515ff6-41e3-4440-9609-8fef93e75a82`, and `9f8ecb57-29d3-4713-a07e-cc01b532db95`; refreshed by Master 2026-05-28 19:43 UTC):**

| count | jurisdiction(s) | pipeline line | class | status |
|---:|---|---|---|---|
| route gap | bbox refresh operator endpoint | `POST /api/admin/jurisdictions/{id}/refresh-bbox` | **C1 missing operator/API route:** prod returns 404 and route is absent from OpenAPI/current FastAPI route search | Lane A triage; not a spatial data blocker |
...
|---|---|---|---|---|
| A — Integrator | triage missing prod refresh-bbox operator route | none | C1 route gap; B8 reopened with Nassau parked; B7/B6 cleared | 2026-05-28 20:16 UTC (Lane C no-op) |
...
| C1 | Missing prod `refresh-bbox` operator route (`POST /api/admin/jurisdictions/{id}/refresh-bbox` returns 404; route absent from OpenAPI/current FastAPI route search) | Lane A | operator API hygiene only; not the cause of bbox NULL rows | **TRIAGE NEEDED** — Lane C bbox sweep updated 0 because all 7 remaining bbox-null jurisdictions have `parcel_count=0` and `geom_count=0` |
