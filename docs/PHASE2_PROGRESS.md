...
**Last 14d (sources: `backend/tmp/jobs_all_latest.json`, `backend/tmp/job_monomouth_08b0f866.json`, `backend/tmp/job_westchester_886141e2.json`, live admin jobs `d98324cc-6c78-4e2f-a6ab-d6fb15f92835`, `3b4582c5-e47e-4bc7-8d38-7853e0173a89`, and `a9515ff6-41e3-4440-9609-8fef93e75a82`; refreshed by Lane A 2026-05-28 19:05 UTC):**

| count | jurisdiction(s) | pipeline line | class | status |
|---:|---|---|---|---|
| 2 fatal + 1 non-fatal warning | Westchester County NY / Nassau County NY / Monmouth County NJ | Westchester `pipeline.py:1752`; Nassau `pipeline.py:1785`; Monmouth `run_overlays` warning | **B6 confirmed expanded:** post-overlay/run_overlays commit/session reliability; Monmouth warning is residual non-fatal/log-only because job reached `ready` and `complete_feasibility` completed | code resolved by PR #119; Monmouth needs no retest; proceed to Westchester isolated validation |
...
| cleared on Nassau + Monmouth | Nassau County NY / Monmouth County NJ; New York NY same signature | `pipeline.py:1680` -> `bootstrap_zone_use_matrix` -> `matrix_bootstrap.py:70` | **B7 cleared on Monmouth:** hard bootstrap failure did not recur; Monmouth job `a9515ff6` reached `ready` | no Monmouth retest needed |
...
|---|---|---|---|---|
| A — Integrator | Monmouth run_overlays warning classified no-code/log-only; B6 can proceed to Westchester isolated validation | none | B6 pending Westchester validation; B8 reopened with Nassau parked; B7 cleared on Monmouth | 2026-05-28 19:05 UTC (Monmouth `a9515ff6` ready) |
| B — Discovery + Coverage | Westchester County NY isolated B6 validation; Burlington per-town pilot remains separate | — | Nassau/Middlesex/Fairfield parked under B8; Monmouth needs no retest | 2026-05-28 19:05 UTC (Westchester authorized) |
...
| B6 | Westchester post-overlay `db.commit()` failure at `pipeline.py:1752`; Nassau `d98324cc-6c78-4e2f-a6ab-d6fb15f92835` `run_overlays` fatal `db.commit()` failure at `pipeline.py:1785`; Monmouth `a9515ff6-41e3-4440-9609-8fef93e75a82` non-fatal `run_overlays` transaction warning | Lane A | Westchester is now the isolated validation gate | **PATCHED / DEPLOYED / MONMOUTH LOG-ONLY WARNING** — Monmouth reached `ready`; warning is not a blocker/new class; proceed to Westchester only |
