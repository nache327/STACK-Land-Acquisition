...
|---:|---|---|---|---|
| 1 | Nassau County, NY | `pipeline.py:1229` -> `existing_count = await db.scalar(...)` | **B9 confirmed:** forced retry ran unnecessary existing-parcel cache count and failed before parcel counters/download | narrow Lane A patch in progress on `fix/b9-nassau-download-existing-count`; Nassau retry blocked until merge + deploy |
...
|---|---|---|---|---|
| A — Integrator | B9 Nassau forced-retry cache preflight failure classified; narrow forced-run count bypass in progress | fix/b9-nassau-download-existing-count | B9 active until patch merge/deploy; B6 active separate follow-up; B8 parked separate follow-up | 2026-05-26 22:54 UTC (Nassau `3c7ce534`; `/health.pipeline_version: ab88dba1ef6b`) |
...
| B9 | Nassau forced validation retry `3c7ce534-ce81-461e-bab5-76eb64e0105f` failed during `download_parcels` before parcel counters at `pipeline.py:1229 existing_count = await db.scalar(...)`; job `force=true` made the cache count unnecessary | Lane A | Nassau cannot validate B7 until forced-run cache preflight is bypassed | **CONFIRMED; FIX IN PROGRESS** — branch `fix/b9-nassau-download-existing-count` |
