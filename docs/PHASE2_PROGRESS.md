...
| B6 | Westchester post-overlay `db.commit()` failure at `pipeline.py:1752`; Nassau `d98324cc-6c78-4e2f-a6ab-d6fb15f92835` `run_overlays` fatal `db.commit()` failure at `pipeline.py:1785`; Monmouth `a9515ff6-41e3-4440-9609-8fef93e75a82` non-fatal `run_overlays` transaction warning | Lane A | Westchester is now the isolated validation gate | **PATCHED / DEPLOYED / MONMOUTH LOG-ONLY WARNING** — Monmouth reached `ready`; warning is not a blocker/new class; Lane D final queue check is `active_only=0`, `stale_only=0`; proceed to Westchester only |
