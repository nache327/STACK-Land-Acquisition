...
**Last 14d (sources: `backend/tmp/jobs_all_latest.json`, `backend/tmp/job_monomouth_08b0f866.json`, `backend/tmp/job_westchester_886141e2.json`, live admin jobs `91bb9444-377a-4a3e-a3fb-45d7b63ba18e` and `d98324cc-6c78-4e2f-a6ab-d6fb15f92835`; refreshed by Lane A 2026-05-27 21:37 UTC):**

| count | jurisdiction(s) | pipeline line | class | status |
|---:|---|---|---|---|
| 2 | Westchester County NY / Nassau County NY | Westchester `pipeline.py:1752`; Nassau `pipeline.py:1785` -> `await db.commit()` during/after `run_overlays` | **B6 confirmed expanded:** post-overlay/run_overlays commit/session failure after successful ingest and coverage refresh | code resolved by PR #119; Railway deployed `4a081ce429cb`; retry Nassau-only to validate |
...
| cleared on Nassau; Monmouth not run | Nassau County NY / Monmouth County NJ; New York NY same signature | `pipeline.py:1680` -> `bootstrap_zone_use_matrix` -> `matrix_bootstrap.py:70` | **B7 cleared on Nassau:** hard bootstrap failure did not recur before `run_overlays` | Monmouth remains paused until Nassau validates past B6 |
| cleared on Nassau | Middlesex County NJ / Fairfield County CT / Nassau County NY | ingest mapping plateau before bootstrap/overlays | **B8 cleared on Nassau:** job `d98324cc` completed `download_parcels` 420,594 and `ingest_parcels` 420,577 / 17 dedupe with no stale plateau | no broad Middlesex/Fairfield retry until Nassau validates past B6 |
...
|---|---|---|---|---|
| A — Integrator | B6 expanded to Nassau + Westchester post-overlay/run_overlays commit/session failure; PR #119 merged/deployed | PR #119 merged (`4a081ce`) | B6 pending Nassau-only validation; B7/B8/B9/B10 cleared on Nassau; Monmouth paused | 2026-05-27 21:47 UTC (`/health.pipeline_version: 4a081ce429cb`) |
...
| D — Operator + Workflow | stale-job cleanup | — | none for Nassau cleanup; Railway cron-log verification still blocked by expired local CLI auth | 2026-05-28 17:38 UTC (cancelled Nassau `3b4582c5`; final `active_only=0`, `stale_only=0`) |
...
| B6 | Westchester post-overlay `db.commit()` failure at `pipeline.py:1752` plus Nassau `d98324cc-6c78-4e2f-a6ab-d6fb15f92835` `run_overlays` `db.commit()` failure at `pipeline.py:1785` after successful download/ingest/coverage refresh | Lane A | Nassau cannot validate full pipeline; Monmouth should not run yet | **PATCHED / DEPLOYED** — PR #119 merged as `4a081ce429cb1c139d21f55f31fc3c6b88bcdc34`; Railway `/health.pipeline_version=4a081ce429cb`; retry Nassau-only |
...
| B8 | Nassau large-county ingest mapping plateau/stale-lock recurrence (`3b4582c5` cancelled by Lane D after mapping reached `266000 / 420594`, no terminal traceback) | Lane A / Lane B | retry sequencing; Monmouth release | **ACTIVE / CLEANED UP** — Lane D cancelled Nassau `3b4582c5-e47e-4bc7-8d38-7853e0173a89` at `2026-05-28T17:38:22.115015Z`; final `active_only=0`, `stale_only=0`; this cleanup does not release Monmouth |
