...
- **[DONE — Group A re-fire, 3 sub-cov gates cleared, no flip change]** Westchester sub-coverage Group A re-fire per Master's authorization after PR #241 diagnostic. Sprint doc: `docs/OP5_WESTCHESTER_GROUPA_REFIRE.md`. Pass-2-only re-fire on Bedford / Port Chester / Yorktown at `nearest_within_meters=100` via new one-off script `backend/scripts/refire_westchester_groupa_nearest100.py`. Districts already loaded (PR #238); script only touches parcels with `zone_binding_method IS NULL` from PR #238 fire. **All three predictions matched actual to the decimal**: Bedford 68.8 % → 70.16 % (+85), Port Chester 69.1 % → 76.14 % (+382), Yorktown 67.3 % → 71.40 % (+585). All three munis now clear both gates (cov ≥70%, near share <30%). Westchester county-wide coverage 85.96 % → **86.37 %** (+1,052 parcels, +0.41 pp); nearest share 4.62 % → 5.07 %. Group B (North Salem, Somers) left as-is per PR #241 recommendation — no nearest threshold clears both gates. ONE refresh fired — client timed out at 200s (Railway proxy ceiling), did not retry. DB-level numbers authoritative. **Operational count unchanged: 19** (Westchester is already operational — this is polish, not a flip dispatch).
