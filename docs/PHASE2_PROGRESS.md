...
**Last 14d (sources: `backend/tmp/jobs_all_latest.json`, `backend/tmp/job_monomouth_08b0f866.json`, `backend/tmp/job_westchester_886141e2.json`, live admin jobs `d98324cc-6c78-4e2f-a6ab-d6fb15f92835` and `3b4582c5-e47e-4bc7-8d38-7853e0173a89`; refreshed by Master 2026-05-28 17:38 UTC):**

| count | jurisdiction(s) | pipeline line | class | status |
|---:|---|---|---|---|
| 2 | Westchester County NY / Nassau County NY | Westchester `pipeline.py:1752`; Nassau `pipeline.py:1785` -> `await db.commit()` during/after `run_overlays` | **B6 confirmed expanded:** post-overlay/run_overlays commit/session failure after successful ingest and coverage refresh | code resolved by PR #119; validation gate revised to Monmouth County NJ after Nassau recurred as B8 |
...
| cleared on Nassau; Monmouth authorized | Nassau County NY / Monmouth County NJ; New York NY same signature | `pipeline.py:1680` -> `bootstrap_zone_use_matrix` -> `matrix_bootstrap.py:70` | **B7 cleared on Nassau:** hard bootstrap failure did not recur before `run_overlays` | run Monmouth County NJ only for independent validation |
...
|---|---|---|---|---|
| A — Integrator | B6 expanded to Nassau + Westchester post-overlay/run_overlays commit/session failure; PR #119 merged/deployed | PR #119 merged (`4a081ce`) | B6 pending Monmouth validation; B8 reopened with Nassau parked; B9/B10 cleared on Nassau | 2026-05-28 17:38 UTC (Master decision) |
| B — Discovery + Coverage | Monmouth County NJ only after Lane D Nassau cleanup; Burlington per-town pilot remains separate | — | Nassau/Middlesex/Fairfield parked under B8; Westchester held | 2026-05-28 17:38 UTC (Monmouth authorized) |
...
| B6 | Westchester post-overlay `db.commit()` failure at `pipeline.py:1752` plus Nassau `d98324cc-6c78-4e2f-a6ab-d6fb15f92835` `run_overlays` `db.commit()` failure at `pipeline.py:1785` after successful download/ingest/coverage refresh | Lane A | Monmouth is now the validation gate; Westchester remains held | **PATCHED / DEPLOYED / PENDING MONMOUTH VALIDATION** — PR #119 merged as `4a081ce429cb1c139d21f55f31fc3c6b88bcdc34`; unblock condition changed from Nassau-only to Monmouth County NJ validation past `run_overlays` |
