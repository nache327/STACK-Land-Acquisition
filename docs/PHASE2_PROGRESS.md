...
|---:|---|---|---|---|
| 2 | Westchester County NY / Nassau County NY | Westchester `pipeline.py:1752`; Nassau `pipeline.py:1785` -> `await db.commit()` during/after `run_overlays` | **B6 confirmed expanded:** post-overlay/run_overlays commit/session failure after successful ingest and coverage refresh | patch in progress: contain overlay commit/session failure as non-fatal and refresh session before continuing |
...
|---|---|---|---|---|
| A — Integrator | B6 expanded to Nassau + Westchester post-overlay/run_overlays commit/session failure; narrow patch in progress | pending | B6 active expanded; B7/B8/B9/B10 cleared on Nassau; Monmouth paused | 2026-05-27 21:37 UTC (Nassau `d98324cc`: failed at `pipeline.py:1785 await db.commit()` during `run_overlays`) |
...
| D — Operator + Workflow | stale-job cleanup | — | none for Nassau cleanup; Railway cron-log verification still blocked by expired local CLI auth | 2026-05-27 17:46 UTC (cancelled Nassau `91bb9444`; final `active_only=0`, `stale_only=0`) |
...
| ~~B8~~ | ~~Middlesex + Fairfield + Nassau large-county ingest mapping plateau/stale-lock class (`parcels_mapped` 114,000 / 122,000 cancelled; Nassau `91bb9444` stale at `320000 / 420594` on attempt 2 and `150000 / 420594` on attempt 3, no terminal traceback)~~ | ~~Lane A / Lane B~~ | ~~retry sequencing~~ | **CLEARED ON NASSAU** — job `d98324cc` completed `download_parcels` 420,594 and `ingest_parcels` 420,577 / 17 dedupe; no stale/plateau; no broad Middlesex/Fairfield retry before B6 validation |
