"""LLM-draft per-city zone verdicts for Somerset County, NJ — wealth-pocket sprint.

Twin of scripts/pattern_bergen_nj_parse.py (same fetch -> parse -> keyed JSON ->
review -> apply flow); only the county config below differs. See that file for
the full rationale on the keyed output shape, the 80k->200k truncation cap, and
the eCode360 chapter-crawl / multi-URL handling.

Targets (parse + apply — all 100% zoning_code coded, so verdicts bind to
thousands of parcels immediately):
  - Bridgewater township  (16,445 parcels)
  - Franklin township     (22,746 parcels)
  - Bound Brook borough     (2,665 parcels)
  - Warren township         (6,067 parcels)  [Tier-3]
  - Montgomery township     (7,809 parcels)  [Tier-3]

SKIPPED (already in Somerset matrix, verified live 2026-06-02): Bedminster
township, Hillsborough township, Somerville borough.

Run (dry-run):
  cd backend && .venv/Scripts/python.exe scripts/pattern_somerset_nj_parse.py --dry-run
"""
...
# URLs sourced 2026-06-02 via eCode360 search — targeting the Use-Regulations /
# Zoning chapter node (NOT the code root) per the Bergen lesson. A city may
# carry `ordinance_url` (single) OR `ordinance_urls` (list, fetched + merged).
CITIES = [
    # Article XLII: Use Regulations (Part 12 Zoning, Ch. 126 Land Use).
    {"municipality": "Bridgewater township", "ordinance_url": "https://ecode360.com/8962074"},
    # Franklin keeps uses in "Schedule 1, Schedule of Permitted Uses" (a table
    # attachment) + Article II district list; Article III alone only yielded 7
    ...
    # Chapter 16: Land Development.
    {"municipality": "Montgomery township", "ordinance_url": "https://ecode360.com/34978977"},
]

# Cities promoted by --apply. All five are 100% coded so every verdict binds.
APPLY_CITIES = (
    "Bridgewater township", "Franklin township", "Bound Brook borough",
    "Warren township", "Montgomery township",
)

OUT_PATH = Path(__file__).parent / "proposed_verdicts_somerset.json"
...
async def run_dry_run(only: set[str] | None = None) -> None:
    ...
    out = {
        "jurisdiction_id": SOMERSET_JID,
        "jurisdiction_name": "Somerset County, NJ",
...
async def run_apply(path: str) -> None:
    ...
    base = f"{API_BASE}/api/jurisdictions/{SOMERSET_JID}/zones"
...
def main() -> None:
    ...
    g.add_argument("--dry-run", action="store_true",
                   help="Fetch+parse, write proposed_verdicts_somerset.json, NO DB writes.")
    g.add_argument("--apply", metavar="FILE",
                   help="Promote a reviewed proposed_verdicts_somerset.json into prod "
                        "zone_use_matrix (all five Somerset target cities).")
