"""Phase 6B.2 PIVOT — Mill Creek per-muni registration + WAZA ingest.
...
Pre-state:
  - Snohomish County, WA jid 2302220a-16b2-4dc7-965e-b78bb65573f3 (PR #267)
  - 6,237 Mill Creek parcels under Snohomish (parcels.city = 'Mill Creek')
  - No existing 'Mill Creek, WA' jurisdiction
  - No Snohomish zoning_districts loaded yet (Phase 6B.2 hasn't fired)
  - No Snohomish matrix rows yet

Spot-check verified WAZA's 5,406-feature anomaly is REAL parcel-level
polygon density (one polygon per parcel ≈ 5,406 / 6,237 = 0.87 ratio,
8 distinct ZoneIDs: LDR, MDR, HDR, OP, CB, MU/HDR, PRD 7200, EGPUV).

Post-state (this script):
  - New Mill Creek, WA jurisdiction registered
  - 6,237 parcels moved Snohomish → Mill Creek
  - 5,406 WAZA zoning_districts INSERTed under Mill Creek (~5 min wall)
...
Hard rules honored:
  ...
  - municipality matches prod_city_value ('Mill Creek', title-case)
...
logger = logging.getLogger("mill_creek_perm_muni")

DIRECTORY_PATH = (
    Path(__file__).resolve().parent.parent / "data" / "snohomish_wa_zoning_directory.json"
)
SNOHOMISH_JID = "2302220a-16b2-4dc7-965e-b78bb65573f3"
MUNI_NAME = "Mill Creek"  # raw_attributes filter (WAZA Jurisdiction)
PROD_CITY_VALUE = "Mill Creek"  # parcels.city filter (PR #233 lesson)
NEW_JURISDICTION_NAME = "Mill Creek, WA"
ARCGIS_PAGE_SIZE = 1000

# Mill Creek bbox sanity range (Snohomish County south, east of Seattle).
# Mill Creek extends south to ~47.78 (WAZA includes the southern unincorporated
# strip adjacent to Mill Creek city limits) — widened from the initial
# (47.83, 47.92) after the first fire's actual extent surfaced 47.78.
BBOX_LON_RANGE = (-122.30, -122.15)
BBOX_LAT_RANGE = (47.77, 47.92)
...
def _load_directory_entry() -> dict[str, Any]:
    ...
    for e in directory:
        if e["muni_name"] == MUNI_NAME:
            return e
    raise SystemExit(f"Mill Creek entry not in {DIRECTORY_PATH.name}")
...
async def _preflight() -> int:
    print("\n=== PRE-FLIGHT: Mill Creek per-muni (pipeline shape) ===\n")
...
async def _fire(nearest_within_meters: float = 50.0) -> int:
    print(f"\n=== FIRE: Mill Creek per-muni registration + WAZA ingest ===\n")
    entry = _load_directory_entry()
    ...
    try:
        ...
        # === Phase A: Register Mill Creek jurisdiction + move parcels ===
        async with conn.transaction():
            ...
            if existing:
                new_jid = existing["id"]
                print(f"  Found existing Mill Creek jurisdiction: {new_jid}")
            else:
                new_jid = uuid.uuid4()
                await conn.execute(
                    """
                    INSERT INTO jurisdictions (id, name, state, county)
                    VALUES ($1::uuid, $2, 'WA', 'Snohomish')
                    """,
            ...
            p_before = await conn.fetchval(
                ...
                SNOHOMISH_JID, PROD_CITY_VALUE,
            )
            print(f"  Pre-move Mill Creek parcels under Snohomish: {p_before:,}")
            status = await conn.execute(
                ...
                """,
                SNOHOMISH_JID, str(new_jid), PROD_CITY_VALUE,
            )
            n_p = int(status.split()[-1]) if status.split() else -1
            print(f"  Moved parcels Snohomish → Mill Creek: {n_p}")

        # === Phase B: WAZA Class A ingest under Mill Creek jurisdiction ===
        features = await _fetch_arcgis_features(entry["zoning_district_source"])
        ...
        if not (lon_lo <= bbox[0] <= lon_hi and lat_lo <= bbox[1] <= lat_hi):
            raise RuntimeError(
                f"bbox {bbox} outside expected Mill Creek range "
        ...
        print(f"\n=== Mill Creek zoning_code distribution ({len(codes)} codes) ===")
        for r in codes:
