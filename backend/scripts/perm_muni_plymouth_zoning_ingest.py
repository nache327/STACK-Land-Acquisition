...
logger = logging.getLogger("plymouth_zoning_ingest")

# Edina, MN jurisdiction (registered in PR #294 Phase 7A.2).
PLYMOUTH_JID = "7cc5f175-6218-4a7d-b196-70f043652968"
MUNI_NAME = "Plymouth"
PROD_CITY_VALUE = "Plymouth"
LAYER_URL = (
    "https://plymap.plymouthmn.gov/webgis/rest/services/"
    "ParcelViewer/MapServer/2"
)
ZONE_CODE_FIELD = "ZONING"
ZONE_NAME_FIELD = "ZONING"  # source doesn't publish separate human name
RAW_PASSTHROUGH = (
    "OBJECTID", "PID", "HOUSE_NUMB", "STREET", "UNIT",
    "ADDRESS", "ZIP", "ZONING", "Acres",
...
def _build_district_rows(
    ...
    for f in features:
        ...
        raw_attributes = {
            ...
            "muni_name": MUNI_NAME,
            "muni_type": "city",
            "publisher": "City of Plymouth MN (plymap.plymouthmn.gov)",
            "vintage": "live",
        }
...
async def _fire(nearest_within_meters: float = 50.0) -> int:
    ...
    try:
        ...
        s2 = await conn.execute(
            ...
            WHERE target.id = sub.parcel_id
            """,
            PLYMOUTH_JID, binding_label, float(nearest_within_meters),
        )
