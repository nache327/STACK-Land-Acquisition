"""Phase 6B.2 PIVOT — Bainbridge Island per-muni registration + WAZA ingest.
...
Pre-state:
  - Kitsap County, WA jid 88a6b339-c1ca-4a32-8f27-808c0f1409f3 (PR #267)
  - 9,796 Bainbridge Island parcels under Kitsap (parcels.city = 'Bainbridge Island')
  - No existing 'Bainbridge Island, WA' jurisdiction
  - No Kitsap zoning_districts loaded yet (Phase 6B.2 hasn't fired for Kitsap)
  - No Kitsap matrix rows yet

Post-state (this script):
  - New Bainbridge Island, WA jurisdiction registered
  - 9,796 parcels moved Kitsap → Bainbridge
  - 76 WAZA zoning_districts INSERTed under Bainbridge
...
Hard rules honored:
  ...
  - municipality matches prod_city_value ('Bainbridge Island', title-case)
...
logger = logging.getLogger("bainbridge_perm_muni")

DIRECTORY_PATH = (
    Path(__file__).resolve().parent.parent / "data" / "kitsap_wa_zoning_directory.json"
)
KITSAP_JID = "88a6b339-c1ca-4a32-8f27-808c0f1409f3"
MUNI_NAME = "Bainbridge Island"  # raw_attributes filter (WAZA Jurisdiction)
PROD_CITY_VALUE = "Bainbridge Island"  # parcels.city filter (PR #233 lesson)
NEW_JURISDICTION_NAME = "Bainbridge Island, WA"
ARCGIS_PAGE_SIZE = 1000

# Bainbridge Island bbox sanity range (Puget Sound / Kitsap Peninsula)
BBOX_LON_RANGE = (-122.65, -122.40)
BBOX_LAT_RANGE = (47.55, 47.75)
...
def _load_directory_entry() -> dict[str, Any]:
    ...
    for e in directory:
        if e["muni_name"] == MUNI_NAME:
            return e
    raise SystemExit(f"Bainbridge entry not in {DIRECTORY_PATH.name}")
...
async def _preflight() -> int:
    print("\n=== PRE-FLIGHT: Bainbridge Island per-muni (pipeline shape) ===\n")
...
async def _fire(nearest_within_meters: float = 50.0) -> int:
    print(f"\n=== FIRE: Bainbridge Island per-muni registration + WAZA ingest ===\n")
    entry = _load_directory_entry()
    ...
    try:
        ...
        # === Phase A: Register Bainbridge jurisdiction + move parcels ===
        async with conn.transaction():
            ...
            if existing:
                new_jid = existing["id"]
                print(f"  Found existing Bainbridge jurisdiction: {new_jid}")
            else:
                new_jid = uuid.uuid4()
                await conn.execute(
                    """
                    INSERT INTO jurisdictions (id, name, state, county)
                    VALUES ($1::uuid, $2, 'WA', 'Kitsap')
                    """,
            ...
            p_before = await conn.fetchval(
                ...
                KITSAP_JID, PROD_CITY_VALUE,
            )
            print(f"  Pre-move Bainbridge parcels under Kitsap: {p_before:,}")
            status = await conn.execute(
                ...
                """,
                KITSAP_JID, str(new_jid), PROD_CITY_VALUE,
            )
            n_p = int(status.split()[-1]) if status.split() else -1
            print(f"  Moved parcels Kitsap → Bainbridge: {n_p}")

        # === Phase B: WAZA Class A ingest under Bainbridge jurisdiction ===
        features = await _fetch_arcgis_features(entry["zoning_district_source"])
        ...
        if not (lon_lo <= bbox[0] <= lon_hi and lat_lo <= bbox[1] <= lat_hi):
            raise RuntimeError(
                f"bbox {bbox} outside expected Bainbridge range "
        ...
        print(f"\n=== Bainbridge zoning_code distribution ({len(codes)} codes) ===")
        for r in codes:
