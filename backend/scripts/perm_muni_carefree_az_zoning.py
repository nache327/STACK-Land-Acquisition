"""Phase 7B.3 — Carefree AZ Town zoning Class A ingest.

Per Diagnostic PR #324 (2026-06-22 re-verification): Carefree surfaced a new
public zoning FeatureServer. PROMOTED from LOW Path B (PR #319 deferral) to
Path A.

Source: services6.arcgis.com/clPWQMwZfdWn4MQZ/.../TOC_Zoning/FeatureServer/104
- 18 polygons
- Zoning field — 11 nonblank codes + 1 blank (filter blanks)
- Distinct: C, GO, L (Resort), R-3, R1-10, R1-18, R1-35, RUPD, Rural-190,
  Rural-43, Rural-70

Carefree jurisdiction (Phase 7B.2 PR #313):
  da313f75-af51-4e5e-b2fb-a315607fb383 (2,991 parcels)
"""
...
logger = logging.getLogger("carefree_az")

JID = "da313f75-af51-4e5e-b2fb-a315607fb383"
MUNI = "Carefree"
LAYER = "https://services6.arcgis.com/clPWQMwZfdWn4MQZ/arcgis/rest/services/TOC_Zoning/FeatureServer/104"
ZCODE = "Zoning"
RAW_KEYS = ("OBJECTID", "Zoning", "LandUse", "Shape__Area", "Shape__Length")
BBOX_LON = (-111.99, -111.86)
BBOX_LAT = (33.79, 33.86)
...
async def _fire(near=50.0):
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        r = await client.get(f"{LAYER}/query", params={
            ...
            "outSR": 4326, "resultRecordCount": 1000, "f": "json",
            "orderByFields": "OBJECTID",
        })
        r.raise_for_status()
        feats = r.json().get("features", [])
    logger.info("fetched %d", len(feats))
    ...
    for f in feats:
        ...
        raw = {"source_url": LAYER, "source_kind": "arcgis_feature_server",
               ...
               "publisher": "Town of Carefree AZ GIS (Diagnostic PR #324 promoted from deferred)"}
        for k in RAW_KEYS:
    ...
    print(f"features={len(feats)} rows={len(rows)} blank_skipped={blank_skipped} distinct={len(distinct)}: {distinct}")
    ...
    try:
        ...
        if not (BBOX_LON[0] <= bbox[0] <= BBOX_LON[1] and BBOX_LAT[0] <= bbox[1] <= BBOX_LAT[1]):
            raise RuntimeError(f"bbox {bbox} outside envelope (lon {BBOX_LON}, lat {BBOX_LAT})")
