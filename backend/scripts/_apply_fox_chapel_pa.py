"""Fox Chapel Borough PA — Bergen catchall × 4 substrate matrix apply.

Pre-stage source: backend/data/wave6_pre_stage/allegheny_fox_chapel.json
Owner: orchestrator
...
Usage:
    # JID already set in KNOWN_JID (parent Fox Chapel Borough, PA)
    python backend/scripts/_apply_fox_chapel_pa.py
...
#   1. FOX_CHAPEL_BOROUGH_PA_JID environment variable (set at fire-time)
#   2. KNOWN_JID hardcoded below (only if jurisdiction already registered
#      in prod per 2026-06-23 probe; otherwise empty string)
KNOWN_JID = "c5e04fa4-08d7-464b-8b74-dd56fc1f3f17"  # parent Fox Chapel Borough, PA per 2026-06-23 probe

JID = os.environ.get("FOX_CHAPEL_BOROUGH_PA_JID") or KNOWN_JID

API_BASE = "https://capable-serenity-production-0d1a.up.railway.app"
PRESTAGE_PATH = (
    Path(__file__).resolve().parent.parent
    / "data" / "wave6_pre_stage" / "allegheny_fox_chapel.json"
)
REFRESH_SOURCE = "wave6-prestage-fox-chapel-borough-2026-06-23"
# === END CONFIGURATION ===
...
def main() -> int:
    ...
    if not JID:
        print(
            ...
            f"or set env var FOX_CHAPEL_BOROUGH_PA_JID to a jurisdiction UUID.",
            file=sys.stderr,
        )
        return 2
