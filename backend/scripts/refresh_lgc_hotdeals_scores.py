...
SELECT p.id AS parcel_id, {verdict_expr(LGC_SLUG)} AS storage_permission,
       ...
       prm.hnw_households,
       prm3.population AS pop_3mi, prm3.sqft_per_capita AS sqft_per_capita_3mi,
...
LEFT JOIN parcel_radial_metrics prm3 ON prm3.parcel_id=p.id AND prm3.radius_miles=3.0
...
def _inputs(r) -> ParcelInputs:
    """Build the scorer's inputs for the LGC lane.

    MUST stay field-complete against buybox_scoring._select_parcels_sql /
    score_jurisdiction: this script UPSERTs into the same
    parcel_buybox_scores rows, so any input omitted here silently overwrites a
    correct score with one computed WITHOUT that factor. It previously dropped
    lane (so the zoning factor was labelled "Storage" on an LGC card and the LGC
    HNW-depth bonus never fired), pop_3mi (dropping the -20 too-rural penalty)
    and sqft_per_capita_3mi.
    """
    return ParcelInputs(
        ...
        # Lane-aware demand inputs (2026-07-22 sweep) — omitted before, which
        # made this script's scores diverge from auto_score_jurisdiction's.
        lane=LGC_SLUG,
        hnw_households=r["hnw_households"],
        pop_3mi=r["pop_3mi"],
        sqft_per_capita_3mi=(
            float(r["sqft_per_capita_3mi"]) if r["sqft_per_capita_3mi"] is not None else None
        ),
...
async def main() -> int:
    ...
    try:
        ...
        # LGC board filter(s) — the lane that needs fresh listed-parcel scores.
        # Keyed on dashboardEnabled (what dashboard_push._dashboard_filters
        # selects), NOT daily_email_enabled: migration 0051 set that flag FALSE
        # on every filter when the board was decoupled from the email, so this
        # script printed "nothing to do" on every cron tick and the LGC board
        # lane's scores were never refreshed.
        filters = await conn.fetch(
            ...
            SELECT bf.id, bf.name, bf.filter_json
             ...
             WHERE uc.slug = $1
               AND (bf.filter_json->>'dashboardEnabled') = 'true'
               AND COALESCE((bf.filter_json->>'requireListed')::bool, false)
            """,
            LGC_SLUG,
        )
        if not filters:
            print("refresh_lgc_hotdeals: no LGC dashboardEnabled requireListed filter — nothing to do")
            return 0
        ...
        for f in filters:
            ...
            scored = []
            for r in src:
                # asset_label="Garage" mirrors score_jurisdiction's
                # `"Garage" if slug == LGC_SLUG else "Storage"` — without it the
                # LGC card's zoning factor read "Storage".
                s = score_for_parcel(_inputs(r), fj, asset_label="Garage")
