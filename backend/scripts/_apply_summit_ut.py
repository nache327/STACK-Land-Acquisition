"""Summit County UT — Bergen catchall × 4 substrate matrix apply.

Pre-stage source: backend/data/wave6_pre_stage/summit_park_city_corridor.json
Owner: orchestrator
...
Usage:
    export SUMMIT_COUNTY_UT_JID=<jurisdiction_uuid>  # resolve from prod
    python backend/scripts/_apply_summit_ut.py
...
#   1. SUMMIT_COUNTY_UT_JID environment variable (set at fire-time)
#   2. KNOWN_JID hardcoded below (only if jurisdiction already registered
#      in prod per 2026-06-23 probe; otherwise empty string)
KNOWN_JID = ""  # NOT REGISTERED in prod per 2026-06-23 probe; set SUMMIT_COUNTY_UT_JID env var at fire-time

JID = os.environ.get("SUMMIT_COUNTY_UT_JID") or KNOWN_JID

API_BASE = "https://capable-serenity-production-0d1a.up.railway.app"
PRESTAGE_PATH = (
    Path(__file__).resolve().parent.parent
    / "data" / "wave6_pre_stage" / "summit_park_city_corridor.json"
)
REFRESH_SOURCE = "wave6-prestage-summit-county-2026-06-23"
# === END CONFIGURATION ===
...
def main() -> int:
    ...
    if not JID:
        print(
            ...
            f"or set env var SUMMIT_COUNTY_UT_JID to a jurisdiction UUID.",
            file=sys.stderr,
        )
        return 2
