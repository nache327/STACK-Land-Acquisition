"""CO Arapahoe + Cherry Hills standalone carry adapter.

Standalone partial-fire slice derived from perm_co_front_range_carry.py
(PR #355, branch adarench/co-front-range-spec, still OPEN). This adapter
fires Arapahoe County only — skipping Douglas + Jefferson — so the
Cherry Hills Village + Greenwood Village wealth-pocket polygons can flip
while PR #355 remains gated on Master review.

When fired, it performs one guarded carry sequence:

1. Register/find the Arapahoe County, CO umbrella jurisdiction.
2. Ingest Colorado Public Parcels statewide records filtered to
   Arapahoe County into the umbrella JID.
3. Register/find the Cherry Hills Village, CO per-muni jurisdiction.
4. Move matching parcels from the Arapahoe umbrella JID to the
   Cherry Hills Village JID via PATH 1 discipline.
5. Ingest Cherry Hills Village zoning (Arapahoe County Zoning layer 89)
   and spatially backfill parcel zoning.

Drift fix vs. PR #355: Arapahoe County Zoning layer 89 OID field
renamed `OBJECTID` → `FID` (last edit 2025-10-23). Preflight on the
trio adapter crashes with `Invalid field: OBJECTID`; this adapter
uses `FID`.

Greenwood Village (Phase 6 secondary, also Arapahoe) is NOT included
here — it fires from the already-merged
perm_muni_greenwood_village_co_zoning.py after this adapter completes.

The `fire` command requires an explicit production-write
acknowledgement flag. Sources documented in
docs/CO_FRONT_RANGE_ACQUISITION_SPEC.md.
"""
...
MUNIS: tuple[MuniConfig, ...] = (
    MuniConfig(
        ...
        source_scope_note=(
            ...
            "Arapahoe layer 89 OID field renamed OBJECTID -> FID "
            "(last edit 2025-10-23, drift fix vs PR #355 baseline)."
        ),
        zoning=ZoningSourceConfig(
