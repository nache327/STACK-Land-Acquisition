"""Buckhead GA — Bergen catchall × 4 substrate matrix apply.

Pre-stage source: backend/data/wave6_pre_stage/fulton_buckhead.json
Owner: orchestrator
...
Usage:
    export BUCKHEAD_GA_JID=<jurisdiction_uuid>  # resolve from prod
    python backend/scripts/_apply_atlanta_buckhead_ga.py
...
#   1. BUCKHEAD_GA_JID environment variable (set at fire-time)
#   2. KNOWN_JID hardcoded below (only if jurisdiction already registered
#      in prod per 2026-06-23 probe; otherwise empty string)
KNOWN_JID = ""  # NOT REGISTERED in prod per 2026-06-23 probe; set BUCKHEAD_GA_JID env var at fire-time

JID = os.environ.get("BUCKHEAD_GA_JID") or KNOWN_JID

API_BASE = "https://capable-serenity-production-0d1a.up.railway.app"
PRESTAGE_PATH = (
    Path(__file__).resolve().parent.parent
    / "data" / "wave6_pre_stage" / "fulton_buckhead.json"
)
REFRESH_SOURCE = "wave6-prestage-buckhead-2026-06-23"
# === END CONFIGURATION ===
...
def main() -> int:
    ...
    if not JID:
        print(
            ...
            f"or set env var BUCKHEAD_GA_JID to a jurisdiction UUID.",
            file=sys.stderr,
        )
        return 2
