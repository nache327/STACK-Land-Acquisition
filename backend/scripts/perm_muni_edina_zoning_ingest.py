...
logger = logging.getLogger("edina_zoning_ingest")

# Edina, MN jurisdiction (registered in PR #294 Phase 7A.2).
EDINA_JID = "2b08fa13-bc49-489d-9735-bfff7f297352"
MUNI_NAME = "Edina"
PROD_CITY_VALUE = "Edina"
LAYER_URL = (
    "https://services3.arcgis.com/rNrGj3CxKnr9E71f/arcgis/rest/services/"
    "2026_04_13_Existing_Map_Edina_WFL1/FeatureServer/1"
)
ZONE_CODE_FIELD = "E_Zoning"
ZONE_NAME_FIELD = "E_Zoning"  # source doesn't publish separate human name
RAW_PASSTHROUGH = (
    "OBJECTID_1", "PID", "HOUSE_NO", "STREET_NM",
    "MAILING__1", "ZIP_CD", "E_Zoning", "P_Zoning",
    "LotSizeSubarea", "HeightOverlay",
)
ARCGIS_PAGE_SIZE = 1000

# Edina bbox sanity range (slightly wider than Phase 7A.2's per-muni bbox).
BBOX_LON_RANGE = (-93.45, -93.28)
BBOX_LAT_RANGE = (44.85, 44.95)
...
def _rings_to_wkt(rings: list[list[list[float]]]) -> str:
    ...
    ST_MakeValid. Avoids the ring-winding heuristic that mis-classified
    TIGER 2020's Tacoma polygon."""
    ring_wkts = []
    for r in rings:
...
def _build_district_rows(
    ...
    for f in features:
        ...
        raw_attributes = {
            ...
            "muni_name": MUNI_NAME,
            "muni_type": "city",
            "publisher": "ZoneCo (Edina planning consultant)",
            "vintage": "2026-04-13",
        }
...
async def _fire(nearest_within_meters: float = 50.0) -> int:
    ...
    try:
        ...
        s2 = await conn.execute(
            ...
            WHERE target.id = sub.parcel_id
            """,
            EDINA_JID, binding_label, float(nearest_within_meters),
        )
