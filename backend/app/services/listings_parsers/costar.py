...
class Parser:
    ...
    def parse_dataframe(cls, df, filename: str) -> ParseResult:
        ...
        # CoStar exports "Name" (the building name); "Property Name" / "Building Name" are aliases from other templates
        col_pname    = pick_column(cols, "Name", "Property Name", "Building Name")
        col_land_ac  = pick_column(cols, "Land Area (AC)", "Land Area AC", "Land Area Acres")
        col_land_sf  = pick_column(cols, "Land Area (SF)", "Land Area SF")
        col_ppac     = pick_column(cols, "Price Per AC Land", "Price Per Acre", "Price Per AC")
        ...
        col_subm     = pick_column(cols, "Submarket", "Submarket Name")
        ...
        for _, row in df.iterrows():
            ...
            rows.append(ListingRow(
                ...
                last_sale_date=to_date(row.get(col_lsale_d)) if col_lsale_d else None,
